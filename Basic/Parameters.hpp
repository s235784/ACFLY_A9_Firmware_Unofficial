...
class parametersIterator
{
	...
	public:
		...
		*/
		bool reset();
		//迭代器是否可用(初始化成功才可用)
		bool available() { return this->it!=0; }
		void setInAvailable();
		//迭代器自增
		...
		parametersIterator & operator--();
		//迭代器比较
		bool operator==(parametersIterator &ob);
		bool operator!=(parametersIterator &ob);
		//迭代器赋值
		parametersIterator &operator=(const parametersIterator& s);
		...
		返回值：
			...
			PR_OK：操作成功
		*/
		PR_RESULT read( SName* name, uint32_t* index, MAV_PARAM_TYPE* type, uint64_t* value, bool* is_new, double TIMEOUT=-1 );
