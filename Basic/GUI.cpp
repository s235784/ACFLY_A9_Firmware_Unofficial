...
static void GUI_Server(void* pvParameters)
{
	...
	// nuotian
	DrawFont16x8( "MSafe:", 32, 100, BLACK );
	DrawFont16x8( "ACM:", 48, 100, BLACK );
	DrawFont16x8( "PCM:", 64, 100, BLACK );
	DrawFont16x8( "Mode:", 80, 100, BLACK );
	DrawFont16x8( "Time:", 96, 100, BLACK );
	DrawFont16x8( "MSG:", 112, 100, BLACK );
	DrawFont16x8( "EXA:", 128, 100, BLACK );
	DrawFont16x8( "Data:", 144, 100, BLACK );
	DrawFont16x8( "RCT:", 160, 100, BLACK );
	...
	while(1)
	{
		...
		/*XY轴*/
			...
			// 调试信息输出
			ImageFill(
				(void*)&ImgBuffer[32*240+148], ImgFormat_RGB565, 240-10*8,
				MAGENTA,
			10*16, 10*8);
			// MSafe状态
			bool msafe_en;
			get_MSafe_en(&msafe_en);
			bool isMSafe = (xTaskGetCurrentTaskHandle()==MSafeTaskHandle);
			bool ForceMSafeCtrl = is_MSafeCtrl();
			sprintf( str, "E:%d S:%d,%d", msafe_en, isMSafe, ForceMSafeCtrl );
			DrawFont16x8( str, 32, 150, WHITE );
			// 高度控制模式
			Position_ControlMode aMode;
			get_Altitude_ControlMode(&aMode);
			sprintf( str, "%d", aMode );
			DrawFont16x8( str, 48, 150, WHITE );
			// 水平控制模式
			Position_ControlMode pMode;
			get_Position_ControlMode(&pMode);
			sprintf( str, "%d", pMode );
			DrawFont16x8( str, 64, 150, WHITE );
			// 控制模式
			int mode;
			get_m32_debug_mode(&mode);
			sprintf( str, "%d", mode );
			DrawFont16x8( str, 80, 150, WHITE );
			// 进入Offboard的持续时间
			unsigned int lasting;
			get_m32_offboard_lasting(&lasting);
			sprintf( str, "%u", lasting );
			DrawFont16x8( str, 96, 150, WHITE );
				
			// debug
			int m32_msg;
			int m32_msg_extra;
			float m32_msg_data;
			get_m32_debug_msg(&m32_msg, &m32_msg_extra, &m32_msg_data);
			sprintf( str, "%d", m32_msg );
			DrawFont16x8( str, 112, 150, WHITE );
			sprintf( str, "%d", m32_msg_extra );
			DrawFont16x8( str, 128, 150, WHITE );
			sprintf( str, "%f", m32_msg_data );
			DrawFont16x8( str, 144, 150, WHITE );
			// RC调用次数
			int rc_time;
			int rc_extra;
			float rc_data;
			get_RCProcess_debug(&rc_extra, &rc_data, &rc_time);
			sprintf( str, "%d", rc_time );
			DrawFont16x8( str, 160, 150, WHITE );
//			sprintf( str, "%d", rc_extra );
//			DrawFont16x8( str, 128, 150, WHITE );
//			sprintf( str, "%f", rc_data );
//			DrawFont16x8( str, 144, 150, WHITE );
