	...
	*/
	...
	/*水平位置*/
		...
		/*直线飞行*/
			...
			bool Position_Control_set_WpRange( double range, double TIMEOUT )
			{
				if( !isvalid(range) )
					return false;
...
//			}		
		...
		TD3filter[0].track3(T3.x, h, get_RolTD4P1(),get_RolTD4P2(),get_RolTD4P3());
		TD3filter[1].track3(T3.y, h, get_RolTD4P1(),get_RolTD4P2(),get_RolTD4P3());
...
PosCtrl_Finish:	
	...
	{//高度控制
		...
		else
		{
			...
			throttle = constrain( throttle + h * 35, 0.0, 100.0 );
			Attitude_Control_set_Throttle( throttle );
		}
