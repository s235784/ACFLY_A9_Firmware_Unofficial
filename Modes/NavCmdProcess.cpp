...
{	
	...
	switch( inf->counter1 )
	{
		...
		{	//降落
			...
			#define max_acc 50.0
			double maxLandVel = sp[0];
//			if( params[1] == PRECISION_LAND_MODE_DISABLED )
//			{	//不需要精准降落
				Position_Control_set_XYLock();
//			}
//			else if( params[1] == PRECISION_LAND_MODE_REQUIRED )
//			{	//精准降落
//				Position_Sensor_Data sensor;
//				if( GetPositionSensorData( default_uwb_sensor_index, &sensor ) )
//				{
//					//读取模式配置
//					ModeFuncCfg MFunc_cfg;
//					ReadParamGroup( "MFunc", (uint64_t*)&MFunc_cfg, 0 );
//					
//					vector2<double> posErr;
//					posErr.x = MFunc_cfg.px[0] - sensor.position.x;
//					posErr.y = MFunc_cfg.py[0] - sensor.position.y;
//					if( posErr.get_square() > sq(80) )
//						maxLandVel = -1;
//					
//					posErr.constrain(120);
...
//			}
			...
			double land_vel = get_LandVel();
			double TVel = land_vel;
			if( height > 1000 + ground_height + sp[0]*2 )
				TVel = sp[0];
			else if( height > 1000 + ground_height )
				TVel = (height-1000-ground_height)*(sp[0]-land_vel)/sp[0]/2 + land_vel;
			else
				TVel = land_vel;
			if( maxLandVel <= 0 )
				Position_Control_set_ZLock();
			else
			{
				if( TVel > maxLandVel )
					TVel = maxLandVel;
				Position_Control_set_TargetVelocityZ(-TVel);
			}
